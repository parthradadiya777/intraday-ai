...
_NIFTY50_PANEL = '''
<div class="panel"><h2>🇮🇳 NIFTY 50</h2><div id="niftyIndex" class="nifty-index">Loading NIFTY 50…</div><div class="searchrow"><input id="niftySearch" class="search" placeholder="🔍 Search NIFTY 50 stock"><span id="niftyInfo" class="searchinfo"></span></div><div class="tablewrap"><table><thead><tr><th>Stock</th><th>Price</th><th>Change</th><th>Weight</th><th>Volume</th><th>Turnover</th></tr></thead><tbody id="niftyRows"><tr><td colspan="6" class="empty">Loading…</td></tr></tbody></table></div></div>
'''
_NIFTY50_SCRIPT = '''<style>.nifty-index{display:flex;gap:18px;align-items:center;flex-wrap:wrap;background:#f6f8fa;border-radius:12px;padding:14px;margin-bottom:14px}.nifty-main{font-size:24px;font-weight:900}.nifty-change{font-size:18px;font-weight:800}.nifty-meta{font-size:12px;color:#687386}</style><script>
let NIFTY50=[];function niftyMoney(x){return x==null?'—':'₹'+Number(x).toLocaleString('en-IN',{minimumFractionDigits:2,maximumFractionDigits:2});}
function niftyRender(){const q=($('niftySearch').value||'').trim().toUpperCase();const rows=NIFTY50.filter(x=>!q||x.symbol.toUpperCase().includes(q));$('niftyInfo').textContent='Showing '+rows.length+' of '+NIFTY50.length+' NIFTY 50 stocks';$('niftyRows').innerHTML=rows.map(x=>{const ch=x.change==null?null:Number(x.change);const cls=ch!=null?(ch>=0?'green':'red'):'';return '<tr><td><b>'+x.symbol+'</b></td><td>'+niftyMoney(x.price)+'</td><td class="'+cls+'">'+(ch==null?'—':ch.toFixed(2)+'%')+'</td><td>'+(x.weightage==null?'—':Number(x.weightage).toFixed(2)+'%')+'</td><td>'+(x.volume==null?'—':Number(x.volume).toLocaleString('en-IN'))+'</td><td>'+(x.turnover==null?'—':Number(x.turnover).toLocaleString('en-IN'))+'</td></tr>';}).join('')||'<tr><td colspan="6" class="empty">No matching NIFTY 50 stock.</td></tr>';}
async function loadNifty50(){try{const [ir,cr]=await Promise.all([fetch('/api/nifty50/index'),fetch('/api/nifty50')]);const ij=await ir.json(),cj=await cr.json();if(ij.ok){const d=ij.data,ch=Number(d.changepct||d.change||0);$('niftyIndex').innerHTML='<span class="nifty-main">NIFTY 50 '+niftyMoney(d.close)+'</span><span class="nifty-change '+(ch>=0?'green':'red')+'">'+(ch>=0?'+':'')+ch.toFixed(2)+'%</span><span class="nifty-meta">O '+niftyMoney(d.open)+' · H '+niftyMoney(d.high)+' · L '+niftyMoney(d.low)+' · Prev '+niftyMoney(d.previous_close)+'</span>';}if(cj.ok){NIFTY50=cj.rows||[];niftyRender();}else $('niftyRows').innerHTML='<tr><td colspan="6" class="empty">'+(cj.error||'NIFTY 50 data unavailable')+'</td></tr>';}catch(e){$('niftyIndex').textContent='NIFTY 50 connection error';}}
$('niftySearch').addEventListener('input',niftyRender);loadNifty50();setInterval(loadNifty50,15000);</script>'''
app_auto.HTML = app_auto.HTML.replace('</body></html>', _NIFTY50_PANEL + _NIFTY50_SCRIPT + '</body></html>')
