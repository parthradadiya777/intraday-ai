...
# Primary fallback: TradingView public India scanner (NSE universe).
...
try:
    ...
    def _tradingview_snapshot():
        ...
        for item in obj.get('data') or []:
            ...
            if exchange.upper() not in ('NSE', 'NSEI'):
                continue
            try:
        ...
        if not rows:
            raise RuntimeError('TradingView India scanner returned no NSE rows')
        return pd.DataFrame(rows)

    def _snapshot(*args, **kwargs):
        # Try nsemine first. If Render/NSE blocks it or returns an empty frame,
        # use TradingView's public India scanner, which covers the NSE universe.
        try:
    ...
    # Never prevent the application from starting because of fallback setup.
    pass
