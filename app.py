...
def patch_stock_modal():
    ...
    new = r"""$('analysisCards').innerHTML='<div class="analysis-grid"><div class="analysis-box"><b>AI SCORE</b><span>'+val(x.score)+'</span></div><div class="analysis-box"><b>CONFIDENCE</b><span>'+val(x.ai_confidence)+'%</span></div><div class="analysis-box"><b>SIGNAL</b><span>'+val(x.signal)+'</span></div></div><div class="trade-grid"><div class="trade-box"><b>LTP</b><span>'+money(x.price)+'</span></div><div class="trade-box"><b>CHANGE</b><span class="'+(Number(x.change)>=0?'green':'red')+'">'+val(x.change)+'%</span></div><div class="trade-box"><b>TARGET</b><span class="green">'+money(x.target)+'</span></div><div class="trade-box"><b>STOP LOSS</b><span class="red">'+money(x.sl)+'</span></div></div><div class="analysis-reason"><b>AI Reason</b><div style="margin-top:7px">'+(x.ai_reason||'Live NSE snapshot with 5-minute AI refinement.')+'</div></div>';$('details').innerHTML='';$('dreason').textContent=''"""
    if old in html:
