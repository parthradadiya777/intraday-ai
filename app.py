...
def settings():
    ...
    except:return {'bot_token':'','chat_id':'','alerts':True}
...
def alert(key,text):
    if not LAST_ALERT.get(key) and telegram(text):LAST_ALERT[key]=1
...
def http_json(url,timeout=12):
    q=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 IntradayAI','Accept':'application/json'})
...
def spark_data():
    ...
    url='https://query1.finance.yahoo.com/v7/finance/spark?symbols='+syms+'&range=1d&interval=1m'
    return http_json(url,timeout=15)

def make_row(symbol,closes):
    a=[float(x) for x in closes if x is not None]
    if len(a)<25:raise ValueError('not enough candles')
    p=a[-1];prev=a[-2];e9=ema(a,9);e21=ema(a,21);rr=rsi(a);change=(p/a[0]-1)*100;mom=(p/prev-1)*100
    bull=0;bear=0
    if p>e9:bull+=18
    else:bear+=18
    if p>e21:bull+=18
    ...
    return {'symbol':symbol,'price':round(p,2),'change':round(change,2),'momentum':round(mom,3),'volume':0,'score':round(score,1),'signal':signal,'ema9':round(e9,2),'ema21':round(e21,2),'rsi':round(rr,1) if rr is not None else None,'vwap':round(p,2),'source':'Yahoo Spark'}

def scan():
    data=[]
    try:
        raw=spark_data()
        results=raw.get('spark',{}).get('result',[])
        by={}
        for item in results:
            sym=item.get('symbol','').replace('.NS','')
            resp=item.get('response',[])
            q=(resp[0].get('indicators',{}).get('quote',[{}])[0] if resp else {})
            by[sym]=q.get('close',[])
        for s in SYMBOLS:
            try:data.append(make_row(s,by[s]))
            except:data.append({'symbol':s,'error':True})
    except Exception as e:
        data=[{'symbol':s,'error':True,'error_detail':str(e)[:120]} for s in SYMBOLS]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith('/api/scan'):
            a=scan();self.send_json({'data':a,'meta':{'ok':sum(not x.get('error') for x in a),'total':len(a)}});return
        if self.path.startswith('/api/test'):
            self.send_json({'ok':telegram('✅ Intraday AI Telegram test successful.')});return
        if self.path.startswith('/api/settings'):self.send_json(settings());return
        self.send_error(404)
    def do_POST(self):
        if self.path.startswith('/api/settings'):
            ...
            save_settings({'bot_token':str(x.get('bot_token','')).strip(),'chat_id':str(x.get('chat_id','')).strip(),'alerts':True});self.send_json({'ok':True});return
        self.send_error(404)

def main():
    p=int(os.environ.get('PORT','10000'));ThreadingHTTPServer(('0.0.0.0',p),Handler).serve_forever()
